...
#include "driver/pcnt.h"                                                  // Biblioteca ESP32 PCNT
          ...
          #define LCD_OFF                                                           // Defina LCD_ON, para usar LCD, se n�o, defina LCD_OFF
          #define LCD_I2C_OFF                                                       // Defina LCD_I2C_ON, para usar LCD I2C, se n�o, defina LCD_I2C_OFF
          ...
          LiquidCrystal_PCF8574 lcd(0x3F);                                          // Instancia LCD I2C com endere�o x3F
          #endif                                                                    // LCD I2C
...
bool            flag          = true;                                     // Indicador de fim de contagem - libera impress�o
...
static void IRAM_ATTR pcnt_intr_handler(void *arg)                        // Contagem do contador de Overflow
  {
    portENTER_CRITICAL_ISR(&timerMux);                                      // Bloqueia nova interrup��o
    multPulses++;                                                           // Incrementa contador de overflow
    PCNT.int_clr.val = BIT(PCNT_COUNT_UNIT);                                // Limpa indicador de interrup��o
    portEXIT_CRITICAL_ISR(&timerMux);                                       // Libera nova interrup��o
  }
...
//----------------------------------------------------------------------------------------
char *ultos_recursive(unsigned long val, char *s, unsigned radix, int pos) // Formata um n�mero longo de 32 bits com pontos
  {
    int c;
    if (val >= radix)
...
//----------------------------------------------------------------------------------------
char *ltos(long val, char *s, int radix)    // Formata um n�mero longo de 32 bits com pontos
  {
    if (radix < 2 || radix > 36)
      {
        s[0] = 0;
